...
export const meta: MetaFunction = ({ params, location }) => {
	...
	return [
		{ title: `${category} Writeups | Hack@UCF` },
		{
			name: "description",
			content: `CTF writeups in the ${category} category from Hack@UCF members.`,
		},
		generateCanonicalUrl(location.pathname),
	];
};

export default function WriteupsByCategory() {
	const { category } = useParams();
	const decoded = decodeURIComponent(category ?? "");
	const writeups = getWriteupsByCategory(decoded);
			...
			<div className="max-w-6xl mx-auto">
				<div className="flex flex-col items-center justify-center mb-12 mt-8">
					...
					<p className="text-stone-400 mt-4">
						Category: <span className="text-brandGold">{decoded}</span>
					</p>
				</div>
				...
				{writeups.length === 0 ? (
					<p className="text-center text-stone-400 py-12">
						No writeups found in this category.
					</p>
				) : (
